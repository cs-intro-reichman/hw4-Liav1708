...
public class ArrCharOps {
    public static void main(String[] args) {
        ...
        //System.out.println(indexOf(arr2,'o',8)); 
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        char[] value = new char[arr1.length + arr2.length];
        for(int i =0; i < arr1.length;i++){
            value[i] = arr1[i];
        }
        for(int i = 0; i < arr2.length; i++){
            value[i + arr1.length] = arr2[i];
        }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        ...
        for(int i = beginIndex; i < value.length + beginIndex; i++){
            value[i - beginIndex] = arr[i];
        }
        return value;
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        if(equals(arr, check)){
            ...
            for(int i = 0;i < arr.length;i++){
                value += arr[i] * Math.pow(7.0, arr.length - counter);
                counter++;
            }
            return value;
        }
